...
class MyGreedyAgent2(Agent):
    ...
    def addWallsToMap(self, state):
        ...
        for i in range(len(walls)):
            ...
            self.map.setScore(walls[i][0], walls[i][1], '%',0)
        for i in range(self.map.getWidth()):            
            if self.map.getScore == "%":
                self.score = 0
    ...
    def getAction(self, state):
        ...
        for i in range(self.map.getWidth()):
            for j in range(self.map.getHeight()):
                if self.map.getValue(i, j) != '%':
                    self.map.setScore(i, j, ' ', -1)

        #get food locations
        food = api.food(state)
        for i in range(len(food)):
            self.map.setScore(food[i][0], food[i][1], '*', 1)        

        print self.map.value

        if pacman == food:
            self.map.value = self.map.value +1
        if pacman != food:
            self.map.value = self.map.value -1
