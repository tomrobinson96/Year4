...
class Grid:
    ...
    def setValue(self, x, y, value, score):
        self.grid[y][x] = value
...
class MyGreedyAgent1(Agent):
        ...
        for i in range(self.map.getWidth()):
            for j in range(self.map.getHeight()):
                if self.map.getValue(i, j) != '%':
                    self.map.setValue(i, j, ' ', -1)
        food = api.food(state)
        for i in range(len(food)):
            self.map.setValue(food[i][0], food[i][1], '*', 1)
    ...
    def getAction(self, state):
        ...
        randomChoice = api.makeMove(random.choice(legal), legal)

        if randomChoice == Directions.EAST:
                print "0.8 Of East"
                print "0.1 of North"
                print "0.1 of South" 
        if randomChoice == Directions.WEST:
                print "0.8 Of West"
                print "0.1 of North"
                print "0.1 of South" 
        if randomChoice == Directions.NORTH:
                print "0.8 Of North"
                print "0.1 of West"
                print "0.1 of East"
        if randomChoice == Directions.SOUTH:
                print "0.8 Of South"
                print "0.1 of West"
                print "0.1 of East"
