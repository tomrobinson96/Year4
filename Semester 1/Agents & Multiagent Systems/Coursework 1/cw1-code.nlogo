...
to execute-behaviour
   ;;if there is a obstacle in front of the agent move to aoid it
   if detect-obstacle [avoid-obstacle stop]
   ;; if the agent is at the base and needs water, service the agent (add water)
   if at-base and need-water [service-unit]
   ;; If the agent has o water, start moving towards the base
   if need-water [move-towards-base]
   ;; If the agent has water and detects a fire near it, put out one of the fires
   if have-water and detect-fire [put-out-fire]
   ;; If the agent has water and is at the base, start moving randomly away from the base
   if have-water and at-base [move-randomly]
