...
to update-intentions
  ...
  [
     ...
     add-intention "not-be-on-mission" "true"
     add-intention "send-location" "true"
     add-intention "do-nothing" timeout_expired 5 ;; wait 5ms before starting (this is to help the visualisation only)
  ]

  if exist-beliefs-of-type "Closest Unit" ;and exist-beliefs-of-type "fire" ;and current-intention = "not-be-on-mission"
  [
    ;;add-intention "send-closest-location-test" "true"
    ;let current-location (list xcor ycor)
    ;let closest-belief item 2 closest-agent-location
    ;if current-location = last closest-belief
    ;  [
    ;    let fire-location closest-fire-location ;; get the belief that relates to the fire which is closest
    ;    ;remove-belief fire-location ;; remove the belief because we will now add an intention for it
     ...
     ;   add-intention (word "move-towards-dest " coords) (word "at-dest " coords)
      ;]
      add-intention "send-unit-to-fire" "true"
      add-intention "inform-all" "true"
    ]
  if exist-beliefs-of-type "Fire at" and exist-beliefs-of-type "fire"
  [
...
to not-be-on-mission
  if "find-target-fire" = true
  [
    report true
  ]
...
to send-location
  add-intention "send-all-unit-locations" "true"
end

;;; Now have location of closest unit, identify agent on location and move
to identify-closest-unit
...
to update-intentions-sc
  if exist-beliefs-of-type "unit" and current-intention = "do-nothing"
  [
    let agent-location closest-agent-location ;; get the belief that relates to the unit which is closest
   ...
   ; let coords item 1 agent-location ;; get the second element of the list
    ...
    ;;INSERT INTENTIONS
    ;add-intention "look-for-fires" "true" ;; 3.
    add-intention "send-closest-location" "true" ;;
    ;add-intention "inform-all" "true" ;; 2.
  ]
end
...
to look-for-fires
  ...
  ;; 2. send a message to all other agents about the location of the fire
  ;; 3. wait for the fire to go out (either because the tree is dead, or a found agent has extinguished it)
  ;; So we add intentions for each of these behaviours
  ;; Note that the intentions are pushed onto the stack in reverse order so that they are popped off of the stack in the correct order
  add-intention "do-nothing" "fire-out" ;; 3.
  add-intention "inform-all" "true" ;; 2.
...
;; Returns the belief of the closest fire location
to-report closest-agent-location
  let agent-beliefs beliefs-of-type "unit" ;; get all the beliefs relating to unit locations

  let closest-belief first agent-beliefs ;; get the first belief in the list
  let closest-location last closest-belief ;; initialise closest location with the location in first belief in the list
  let closest-x first closest-location ;; x coordinate of location
  let closest-y last closest-location ;; y coordinate of location
  let closest-distance (abs (xcor - closest-x)) + (abs (ycor - closest-y)) ;; calculate Manhatten distance to current closest
  ...
  report closest-belief ;; closest-belief is now the belief that represents the unit location closest to the fire
end
...
to inform-all
  ...
  ;broadcast-to scouters msg
end

to send-all-unit-locations
  let msg create-message "inform" ;; create a message with performative "inform"
  set msg add-content (list "unit" who list xcor ycor) msg ;; set the content of the message to the location of the fire
  broadcast-to scouters msg
  broadcast-to units msg
...
to send-closest-location
  let msg create-message "inform" ;; create a message with performative "inform"
  set msg add-content (list "Closest Unit" last closest-agent-location) msg ;; set the content of the message to the location of the fire
  broadcast-to units msg
  broadcast-to scouters msg
end

to send-unit-to-fire
  let msg create-message "inform"
  set msg add-content (list "Fire at" closest-fire-location) msg ;; set the content of the message to the location of the fire
  set msg add-receiver (units-on last closest-agent-location ) msg
end
