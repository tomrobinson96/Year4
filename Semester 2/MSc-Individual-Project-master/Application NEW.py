...
talkThresh = 50             # Decivel level needed to be reached for talking
...
homeDirectory="/home/pi/Audio Files/"
# Path to the Dropbox-uploaded shell script
uploadPath = "/home/pi/Dropbox-Uploader/dropbox_uploader.sh"
...
# After removing the potentially lengthy silences, add a short amount of sillence 
...
# Retreives files in Dropbox directory in child program and returns them as a list to main program
...
def uploadToDropbox(path, level):
    ...
    if not os.path.exists(homePath):
        ...
        for f in files:                                 
            dropboxUploadOutput("Found File: " + f,level)   
            if not f in dropboxFiles:
                fullAddress = os.path.join(homePath,f)
                relativeAddress = os.path.join(path,f)  
                dropboxUploadOutput("Uploading File: " + f,level+1)   
                if fileUpload(fullAddress, relativeAddress) == 1:
                    dropboxUploadOutput("Uploaded File: " + f,level + 1)                                            
                else:
                    dropboxUploadOutput("Error Uploading File: " + f,level + 1)
                    
        #If conducting a deep search of directories, loop through them too   
        if deepSearch == 1:
            for d in directories:
                dropboxUploadOutput("Found Directory: " + d, level)
...
def record():       
    ...
    while shoutingLoop == 1 :
        ...
        while recordingLoop < 2 :
            ...
            # If time since talking intially heard exceeds 5 seconds, reset timer and tell user no shouting heard
            if talkTime >= 500:
            ...
            #Reset talking timer if any audio is heard as to extend the recording
    ...
    # Process data accoring to classes
    r = averageVolume(r)
    r = concentrateData(r)    
    r = addSilence(r, 0.5)
...
if __name__ == '__main__':   
    # Infinite loop so system will alwayd record if active 
    while restart == 1:
